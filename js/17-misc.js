...
function _mostrarMarco(m) {
  ...
  el.innerHTML = '<div style="font-size:28px;flex-shrink:0">🏆</div>'
    ...
    + '<button onclick="this.closest(\'#marco-toast\').remove()" style="background:none;border:none;font-size:18px;cursor:pointer;color:#999;flex-shrink:0"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px"><path d="M18 6L6 18M6 6l12 12"/></svg></button>';
...
function atualizarTrialUI(count) {
  ...
  if (_tfPlanPro) {
    if (bar) {
      // Plano Fundador (migration 026): 10 primeiros assinantes — selo + promessa
...
function agendaMesDetalhe(day, month, year) {
  ...
  overlay.innerHTML = '<div style="background:var(--white);border-radius:16px;width:100%;max-width:420px;box-shadow:0 24px 64px rgba(0,0,0,.2);overflow:hidden">' +
    '<div style="padding:20px 24px 16px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">' +
      ...
      '<button onclick="document.getElementById(\'modal-mes-detalhe\').remove()" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--muted)"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px"><path d="M18 6L6 18M6 6l12 12"/></svg></button>' +
    '</div>' +
